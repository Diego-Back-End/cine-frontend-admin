...
const BASE_ITEMS = [
  ...
  { to: '/panel-inicio/peliculas', label: 'Películas', icon: <FiFilm className={ICON_CLASS} /> },
  { label: 'Funciones', icon: <FiCalendar className={ICON_CLASS} /> },
]

const SUPER_ADMIN_ITEMS = [
  { label: 'Reportes', icon: <FiBarChart2 className={ICON_CLASS} /> },
  { to: '/panel-inicio/sucursales', label: 'Sucursales', icon: <FiMapPin className={ICON_CLASS} /> },
              ...
              <li key={item.label}>
                {item.to ? (
                  ...
                  <button
                    ...
                    disabled
                    data-tip={item.label}
                    className="menu-active text-white is-drawer-close:tooltip is-drawer-close:tooltip-right"
                  >
                    {item.icon}
                    <span className="is-drawer-close:hidden">{item.label}</span>
                  </button>
